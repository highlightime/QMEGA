...
func ReadAccountTrieNode(db ethdb.KeyValueReader, path []byte) []byte {
	data, _ := db.Get(40, accountTrieNodeKey(path))
	return data
}
...
func HasAccountTrieNode(db ethdb.KeyValueReader, path []byte) bool {
	has, err := db.Has(41, accountTrieNodeKey(path))
	if err != nil {
		return false
	}
	return has
...
func ReadStorageTrieNode(db ethdb.KeyValueReader, accountHash common.Hash, path []byte) []byte {
	data, _ := db.Get(42, storageTrieNodeKey(accountHash, path))
	return data
}
...
func HasStorageTrieNode(db ethdb.KeyValueReader, accountHash common.Hash, path []byte) bool {
	has, err := db.Has(7, storageTrieNodeKey(accountHash, path))
	if err != nil {
		return false
	}
	return has
