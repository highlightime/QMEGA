...
func WriteDatabaseVersion(db ethdb.KeyValueWriter, version uint64) {
	...
	if err = db.Put(191, databaseVersionKey, enc); err != nil {
...
func WriteChainConfig(db ethdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	...
	if err := db.Put(201, configKey(hash), data); err != nil {
...
func WriteGenesisStateSpec(db ethdb.KeyValueWriter, blockhash common.Hash, data []byte) {
	if err := db.Put(211, genesisStateSpecKey(blockhash), data); err != nil {
...
func PushUncleanShutdownMarker(db ethdb.KeyValueStore) ([]uint64, uint64, error) {
	...
	if err := db.Put(221, uncleanShutdownKey, data); err != nil {
...
func PopUncleanShutdownMarker(db ethdb.KeyValueStore) {
	...
	data, _ := rlp.EncodeToBytes(uncleanShutdowns)
	if err := db.Put(231, uncleanShutdownKey, data); err != nil {
...
func UpdateUncleanShutdownMarker(db ethdb.KeyValueStore) {
	...
	data, _ := rlp.EncodeToBytes(uncleanShutdowns)
	if err := db.Put(241, uncleanShutdownKey, data); err != nil {
		log.Warn("Failed to write unclean-shutdown marker", "err", err)
	}
}
...
func WriteTransitionStatus(db ethdb.KeyValueWriter, data []byte) {
	if err := db.Put(251, transitionStatusKey, data); err != nil {
